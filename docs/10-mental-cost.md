...
| baseCost | 條件 | 範例 |
|----------|------|------|
| **1** | 兩數 < 40、個位不進位、和 < 100 | `12 + 15` → **1** |
| **2** | 和 < 150（含需穩定處理進位） | `47 + 68` → **2** |
| **3** | 更大兩位數或三位數、連續進位 | `89 + 76` → **3** |
...
| baseCost | 條件 | 範例 |
|----------|------|------|
| **1** | 被減數 < 40、減數 < 20、個位不借位 | `23 − 12` → **1** |
| **2** | 被減數 < 100（含需借位） | `47 − 28` → **2** |
| **3** | 更大數字、多次借位 | `150 − 87` → **3** |
...
| baseCost | 條件 | 範例 |
|----------|------|------|
| **1** | max ≤ 15 且 min ≤ 9 | `14 × 6` → **1** |
| **2** | max ≤ 19 且 min ≤ 9 | `19 × 7` → **2** |
| **3** | max ≤ 49 且 min ≤ 19 | `37 × 12` → **3** |
| **4** | 更大兩位數乘法 | `42 × 18` → **4** |
...
| baseCost | 條件 | 範例 |
|----------|------|------|
| **1** | 被除數 ≤ 81、除數 ≤ 9 | `16 ÷ 4` → **1** |
| **2** | 被除數 ≤ 144、除數 ≤ 12 | `84 ÷ 12` → **2** |
| **3** | 除數 ≤ 12（需估商） | `168 ÷ 12` → **3** |
| **4** | 更大被除數、除數不友善 | `252 ÷ 14` → **4** |
...
|---|----------|----------|------|-------------|-----|----------------|
| 1 | `base × partner` | `14 × 6` | 乘法 ×1 | 1 | 0 | **1** |
| 2 | `left + right` | `12 + 15` | 加法 ×1 | 1 | 0 | **1** |
| 3 | `dividend ÷ divisor` | `84 ÷ 12` | 除法 ×1 | 2 | 0 | **2** |
| 4 | `(a + b) × c` | `2 × (12 + 15)` | 加法 + 乘法 | 1 + 3 | 1 | **5** |
| 5 | `(a × b) + c` | `(14 × 6) + 8` | 乘法 + 加法 | 1 + 2 | 1 | **4** |
| 6 | `a² − b²` | `23² − 17²` | 平方 + 平方 + 減法 | 3 + 2 + 3 | 2 | **10** |
| 7 | `(a + b)(a − b)` | `(25 + 15)(25 − 15)` | 加法 + 減法 + 乘法 | 2 + 1 + 3 | 2 | **8** |
