...
| 非零 | 「避免零」含：步驟結果 0、零運算元、相鄰互逆抵消、絕對值與偶次方同題分離；~98% 拒絕／~2% 接受；整題最終答案不另設硬性非零。 |
| 絕對值 | 教學意圖改為「絕對值是一種計算方式」，可依難度用更大數字範圍；非零仍以 reroll 為主；與偶次方同題分離。 |
...
「避免零」策略涵蓋下列情況（皆走同一套 ~98% 拒絕／~2% 接受的 reroll 政策，不另開系統）：

1. **步驟結果為 0：** 每個計算步驟的中間結果（`resultForTemplate`）約 **98%** 不為 0（含分數相減得 0，如 `2/3 − 2/3`）。
2. **零運算元：** 加減乘步驟的運算元為 0（例如 `0 + 0.5`、`0 × 8`）視為平凡，應 reroll／拒絕。
3. **相鄰互逆抵消：** 連續兩步若運算互逆且量相同——`+x` 再 `−x`、`−x` 再 `+x`、`×x` 再 `÷x`、`÷x` 再 `×x`——應 reroll／拒絕（本版僅檢查**相鄰**步驟）。
4. **絕對值與偶次方分離：** 同題 `costTemplates` 不得同時出現 `absolute-value` 與偶次方（`square`／`fourth-power`／`decimal-square`）；偶次方已使結果非負，絕對值多餘。
...
2. 若仍反覆違規：以約 **2%** 機率**接受**；其餘情況改抽其他模板（或放棄本次候選並重試整題生成）。

長期統計上，步驟中間結果為 0 的比例應接近 ~2%（互補於 ~98% 非零）。零運算元／相鄰抵消／絕對值+偶次方亦套用同一接受率介面。
...
- **與偶次方：** 見 §6.1 第 4 點；同題分離，避免 `|−a|` 再偶次方的冗餘組合。
...
5. 「避免零」~98%（步驟結果 0、零運算元、相鄰互逆、絕對值+偶次方）；處理順序為 reroll → 少數接受 → 換模板。
6. 絕對值題依難度可用更大數字範圍，體現「計算方式」意圖；同題不與偶次方並存。
