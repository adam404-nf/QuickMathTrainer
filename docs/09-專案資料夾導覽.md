...
| 檔案 | 功能 |
|------|------|
| `types.ts` | 定義 `Question`（題目長什麼樣）、`QuestionType`（四則/分數/冪次）、難度、心算成本等 |
| `templates.ts` | 題目模板庫：例如「接近整十的乘法」「分數加法」等具體出題公式 |
| `constraints.ts` | 品質檢查：題目有沒有重複、心算成本是否超過難度上限、選擇題選項是否合法 |
| `registry.ts` | 題型登記處：統一入口，practice 只跟 registry 說「給我一題」，不用知道細節 |
